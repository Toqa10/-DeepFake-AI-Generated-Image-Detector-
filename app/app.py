...
.step-card{background:#13131f;border:1px solid #1e1e2e;border-radius:12px;padding:14px 16px;}
...
.scan-line{height:2px;background:linear-gradient(90deg,transparent,#7c6af7,transparent);animation:scan 2s infinite;}
...
def load_model():
    ...
    # Try local app dir first, then parent
    model_path = os.path.join(app_dir, 'pixel.pth')
    if not os.path.exists(model_path):
        model_path = os.path.join(parent_dir, 'models', 'pixel.pth')
    mean_path  = os.path.join(app_dir, 'norm_mean.npy')
    if not os.path.exists(mean_path):
        mean_path = os.path.join(parent_dir, 'data', 'norm_mean.npy')
    std_path   = os.path.join(app_dir, 'norm_std.npy')
    if not os.path.exists(std_path):
        std_path = os.path.join(parent_dir, 'data', 'norm_std.npy')
    model = PixelCNN()
...
def get_fft_features(pil_img):
    """Extract FFT magnitude spectrum for visualization"""
...
def predict(pil_img):
    ...
    return pred, float(probs[0]), float(probs[1])  # pred, p_real, p_fake
...
def overlay_gradcam(pil_img, cam_np):
    ...
    cam_img = cam_img.filter(F.GaussianBlur(radius=4))
    # Colorize: low=blue, high=red
...
with tab3:
    ...
    try:
        ...
        if hist:
            ...
            axes[0].plot(eps, hist['tl'], 'o-', color='#7c6af7', ms=5, label='Train')
            axes[0].plot(eps, hist['vl'], 'o-', color='#f472b6', ms=5, label='Val')
            ...
            axes[1].plot(eps, hist['ta'], 'o-', color='#7c6af7', ms=5, label='Train')
            axes[1].plot(eps, hist['va'], 'o-', color='#f472b6', ms=5, label='Val')
    ...
    with st.expander("🏗️ Full Architecture Details"):
        ...
        **PixelCNN Architecture**
        ...
        Input:     (batch, 3, 64, 64)
        ↓
        Block 1:   Conv2d(3→32, k=3) + BN + ReLU + MaxPool(2)  → (32, 32, 32)
        Block 2:   Conv2d(32→64, k=3) + BN + ReLU + MaxPool(2) → (64, 16, 16)
        Block 3:   Conv2d(64→128, k=3) + BN + ReLU             → (128, 16, 16)
        Pool:      AdaptiveAvgPool(4)                           → (128, 4, 4)
        ↓
        Flatten:   2048
        FC1:       2048 → 256 + ReLU + Dropout(0.4)
        FC2:       256  → 64  + ReLU
        FC3:       64   → 2   (REAL / FAKE)
        ↓
        ...
        **Why this architecture detects DeepFakes:**
        - Block 1 learns low-level artifacts (noise, texture inconsistencies)
        - Block 2 learns mid-level patterns (edge inconsistencies, blending artifacts)
        - Block 3 learns high-level semantic inconsistencies
        - Dropout prevents the model from memorizing specific GAN signatures
        """)

st.markdown("<br><p style='text-align:center;color:#1e1e2e;font-size:11px'>Built with PyTorch · Phase 2 of AI/ML Learning Roadmap · CNN + Frequency Analysis + Grad-CAM</p>", unsafe_allow_html=True)
